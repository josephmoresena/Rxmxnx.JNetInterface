...
internal sealed partial class EnvironmentCore : IAccessFeature
{
	...
	public void SetField<TField>(JLocalObject jLocal, JClassObject jClass, JFieldDefinition definition, TField? value)
		where TField : IDataType<TField>
	{
		if (!RuntimeHelpers.IsReferenceOrContainsReferences<TField>())
		{
	...
	public void SetField<TField>(JFieldObject jField, JLocalObject jLocal, JFieldDefinition definition, TField? value)
		where TField : IDataType<TField>, IObject
	{
		ImplementationValidationUtilities.ThrowIfProxy(jField);
		ImplementationValidationUtilities.ThrowIfNotMatchDefinition(definition, jField.Definition);
	...
	{
		using LocalFrame _ = new(this._env, parameterTypes.Length + IVirtualMachine.GetAccessibleDefinitionCapacity);
	...
	public JMethodObject GetReflectedMethod(JMethodDefinition definition, JClassObject declaringClass, Boolean isStatic)
	{
		JObjectLocalRef localRef = this.GetReflectedCall(definition, declaringClass, isStatic);
		return new(this.GetClass<JMethodObject>(), localRef, definition, declaringClass);
	}
	public JConstructorObject GetReflectedConstructor(JConstructorDefinition definition, JClassObject declaringClass)
	{
	...
	public JFieldObject GetReflectedField(JFieldDefinition definition, JClassObject declaringClass, Boolean isStatic)
	{
		...
		JObjectLocalRef localRef =
			nativeInterface.ClassFunctions.ToReflectedField.ToReflected(this.Reference, declaringClass.Reference,
			                                                            fieldId, isStatic);
	...
	public JMethodId GetMethodId(JExecutableObject jExecutable)
	{
		...
		JMethodId result = nativeInterface.ClassFunctions.FromReflectedMethod.FromReflected(this.Reference, localRef);
