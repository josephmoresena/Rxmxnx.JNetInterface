...
internal sealed partial class EnvironmentCache
{
	...
	/// <param name="array">
	/// The one-dimensional array that is the destination of the elements copied from <paramref name="jArray"/>.
	/// </param>
	...
	private unsafe void CopyToPrimitive(JArrayObject jArray, Int32 sizeOf, Array array, Int32 arrayIndex)
	{
		...
		try
		{
			ReadOnlySpan<Byte> span = MemoryMarshal.CreateReadOnlySpan(ref *(Byte*)criticalPtr, sizeOf * jArray.Length);
	...
	/// </param>
	private void CopyToObject<TElement>(JArrayObject jArray, Span<TElement?> span) where TElement : IDataType<TElement>
	{
		using INativeTransaction jniTransaction = this.Host.MemoryManager.CreateTransaction(1);
		JObjectArrayLocalRef arrayRef = jniTransaction.Add<JObjectArrayLocalRef>(jArray);
		for (Int32 i = 0; i < jArray.Length; i++)
	...
	private JObjectLocalRef GetObjectArrayElement(JObjectArrayLocalRef arrayRef, Int32 index)
	{
		...
		JObjectLocalRef localRef =
			nativeInterface.ArrayFunctions.ObjectArrayFunctions.GetObjectArrayElement(this.Reference, arrayRef, index);
		...
		Int32 count = 1)
	{
		...
		switch (signature)
		{
			...
			case CommonNames.ByteSignatureChar:
				...
				arrayFunctions.RegionFunctions.GetByteArrayRegion.Get(this.Reference, jByteArrayRef, index, count,
				                                                      (ValPtr<JByte>)bufferPtr);
				break;
			case CommonNames.CharSignatureChar:
				JCharArrayLocalRef jCharArrayRef = jniTransaction.Add<JCharArrayLocalRef>(jArray);
				arrayFunctions.RegionFunctions.GetCharArrayRegion.Get(this.Reference, jCharArrayRef, index, count,
				                                                      (ValPtr<JChar>)bufferPtr);
				break;
			case CommonNames.DoubleSignatureChar:
				JDoubleArrayLocalRef jDoubleArrayRef = jniTransaction.Add<JDoubleArrayLocalRef>(jArray);
				arrayFunctions.RegionFunctions.GetDoubleArrayRegion.Get(this.Reference, jDoubleArrayRef, index, count,
				                                                        (ValPtr<JDouble>)bufferPtr);
				break;
			case CommonNames.FloatSignatureChar:
			...
			case CommonNames.IntSignatureChar:
				...
				arrayFunctions.RegionFunctions.GetIntArrayRegion.Get(this.Reference, jIntArrayRef, index, count,
				                                                     (ValPtr<JInt>)bufferPtr);
				break;
			case CommonNames.LongSignatureChar:
				JLongArrayLocalRef jLongArrayRef = jniTransaction.Add<JLongArrayLocalRef>(jArray);
				arrayFunctions.RegionFunctions.GetLongArrayRegion.Get(this.Reference, jLongArrayRef, index, count,
				                                                      (ValPtr<JLong>)bufferPtr);
				break;
			case CommonNames.ShortSignatureChar:
		...
		Int32 count = 1)
	{
		...
		switch (signature)
		{
			...
			case CommonNames.DoubleSignatureChar:
				JDoubleArrayLocalRef jDoubleArrayRef = jniTransaction.Add<JDoubleArrayLocalRef>(jArray);
				arrayFunctions.RegionFunctions.SetDoubleArrayRegion.Set(this.Reference, jDoubleArrayRef, index, count,
				                                                        (ReadOnlyValPtr<JDouble>)bufferPtr);
				break;
			case CommonNames.FloatSignatureChar:
				JFloatArrayLocalRef jFloatArrayRef = jniTransaction.Add<JFloatArrayLocalRef>(jArray);
