...
In these projects, `Rxmxnx.JNetInterface` is used via *intermediate libraries* rather than through its packaged `NuGet`
distributions.
...
for all .NET versions and deployment modes supported by `Rxmxnx.JNetInterface`, including **R2R**, **NativeAOT**, and
...
All test projects are built for each .NET version supported by `Rxmxnx.JNetInterface`, targeting the available platforms
and modes for that version:

- **Windows**: x86, x86-64, arm64
- **Linux**: x86-64, arm, arm64
- **macOS**: x86-64, arm64

> **Note**: The **reflection-free mode** is only available in .NET 8.0 and earlier. If a project is not compatible with
...
This application is designed to show the capabilities and potential of using `Rxmxnx.JNetInterface` in .NET applications
that interact with a JVM instance.
...
Additionally, one Java method showcases the main functionality of `Rxmxnx.JNetInterface`:
...
Using `Rxmxnx.JNetInterface`, we define the `JHelloDotnetObject` class representing the Java class
...
`Rxmxnx.JNetInterface` then loads this into a JVM initialized via the `JVirtualMachineLibrary` API.

The native methods of `com.rxmxnx.dotnet.test.HelloDotNet` are handled through a strategy that differentiates the two
implicit concepts of `Rxmxnx.JNetInterface`: the managed abstraction and the unmanaged implementation of JNI.
...
This method serves to demonstrate the adaptive capability of `Rxmxnx.JNetInterface` to correctly identify the managed
type of the returned object through a JNI call.

### Unit Testing

A [test project](./Rxmxnx.JNetInterface.ManagedTest/README.md) was created using Proxy objects to simulate the JVM
behavior in `Rxmxnx.JNetInterface`. The test project runs on the class that implements `IManagedCallback`.

---

# GUI Application Test

This application is designed to demonstrate the capabilities and potential of integrating `Rxmxnx.JNetInterface` into
.NET applications that utilize Swing and AWT graphical components.
...
following a specific interaction pattern. The included JAR file has been compiled with **JDK 1.2**, along with some
minor source code changes to ensure compatibility with that specification.

However, it is recommended to use it with **Java 1.6** or later.
...
is used to build an APK and verify the correct functionality of `Rxmxnx.JNetInterface` on Android.

Additionally, the solution includes the `Rxmxnx.JNetInterface.MobileTest` project, which is a .NET for Android project
that uses `Rxmxnx.JNetInterface.Mobile`. This project demonstrates how the APIs provided by
`Rxmxnx.JNetInterface.Mobile` can coexist and interoperate with `Java.Interop`.