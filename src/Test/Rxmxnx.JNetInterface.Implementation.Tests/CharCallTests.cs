...
public sealed class CharCallTests
{
		...
		JClassObject? jClass = default)
	{
		...
		proxyEnv.Received(isInstance ? 1 : 0).CallCharMethod(localRef, methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
		...
		proxyEnv.Received(isStatic ? 1 : 0)
		        .CallStaticCharMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
