...
public sealed class BooleanCallTests
{
	...
	internal void InstanceTest(CallType callType)
	{
		...
		try
		{
			...
			using JLocalObject jLocal = TestUtilities.CreateString(proxyEnv, BooleanCallTests.fixture.Create<String>());
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(BooleanCallTests.fixture.Create<JClassLocalRef>());
			BooleanCallTests.Test(callType, proxyEnv, jLocal);
		}
		finally
		{
	...
	internal void NonVirtualTest(CallType callType)
	{
		...
		try
		{
			...
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(BooleanCallTests.fixture.Create<JClassLocalRef>());
			BooleanCallTests.Test(callType, proxyEnv, jLocal, JClassObject.GetClass<JThrowableObject>(env));
		}
		finally
		{
			JVirtualMachine.RemoveEnvironment(proxyEnv.VirtualMachine.Reference, proxyEnv.Reference);
	...
	internal void StaticTest(CallType callType)
	{
		...
		try
		{
			...
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(BooleanCallTests.fixture.Create<JClassLocalRef>());
			BooleanCallTests.Test(callType, proxyEnv, default, JClassObject.GetClass<JStackTraceElementObject>(env));
		}
		finally
		{
			JVirtualMachine.RemoveEnvironment(proxyEnv.VirtualMachine.Reference, proxyEnv.Reference);
		...
		JClassObject? jClass = default)
	{
		...
		JFunctionDefinition<JBoolean> def =
			JFunctionDefinition<JBoolean>.Create((CString)BooleanCallTests.fixture.Create<String>(), argsMetadata);
		IObject[] args = callType.GetArgumentsValues(proxyEnv);
		JMethodId methodId = BooleanCallTests.fixture.Create<JMethodId>();
		...
		JBoolean result = BooleanCallTests.fixture.Create<Boolean>();
		...
		proxyEnv.CallBooleanMethod(localRef, methodId, Arg.Is(TestUtilities.GetArgsPtr(args))).Returns(result);
		proxyEnv.CallNonVirtualBooleanMethod(localRef, Arg.Any<JClassLocalRef>(), methodId,
		                                     Arg.Is(TestUtilities.GetArgsPtr(args))).Returns(result);
		proxyEnv.CallStaticBooleanMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Is(TestUtilities.GetArgsPtr(args)))
		        .Returns(result);

		Assert.Equal(
			result,
			!isStatic ?
				JFunctionDefinition<JBoolean>.Invoke(def, jLocal!, jClass, isNonVirtual, args) :
				JFunctionDefinition<JBoolean>.StaticInvoke(def, jClass!, args));
		...
		proxyEnv.Received(isInstance ? 1 : 0).CallBooleanMethod(localRef, methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
		proxyEnv.Received(isNonVirtual ? 1 : 0).CallNonVirtualBooleanMethod(
			localRef, Arg.Any<JClassLocalRef>(), methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
		proxyEnv.Received(isStatic ? 1 : 0)
		        .CallStaticBooleanMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
