...
public sealed class ByteCallTests
{
		...
		JClassObject? jClass = default)
	{
		...
		proxyEnv.Received(isInstance ? 1 : 0).CallByteMethod(localRef, methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
		...
		proxyEnv.Received(isStatic ? 1 : 0)
		        .CallStaticByteMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
