...
public sealed class IntCallTests
{
	...
	internal void InstanceTest(CallType callType)
	{
		...
		try
		{
			...
			using JLocalObject jLocal = TestUtilities.CreateString(proxyEnv, IntCallTests.fixture.Create<String>());
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(IntCallTests.fixture.Create<JClassLocalRef>());
			IntCallTests.Test(callType, proxyEnv, jLocal);
		}
		finally
		{
	...
	internal void NonVirtualTest(CallType callType)
	{
		...
		try
		{
			...
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(IntCallTests.fixture.Create<JClassLocalRef>());
			IntCallTests.Test(callType, proxyEnv, jLocal, JClassObject.GetClass<JThrowableObject>(env));
		}
		finally
		{
			JVirtualMachine.RemoveEnvironment(proxyEnv.VirtualMachine.Reference, proxyEnv.Reference);
	...
	internal void StaticTest(CallType callType)
	{
		...
		try
		{
			...
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(IntCallTests.fixture.Create<JClassLocalRef>());
			IntCallTests.Test(callType, proxyEnv, default, JClassObject.GetClass<JStackTraceElementObject>(env));
		}
		finally
		{
			JVirtualMachine.RemoveEnvironment(proxyEnv.VirtualMachine.Reference, proxyEnv.Reference);
		...
		JClassObject? jClass = default)
	{
		...
		JFunctionDefinition<JInt> def =
			JFunctionDefinition<JInt>.Create((CString)IntCallTests.fixture.Create<String>(), argsMetadata);
		IObject[] args = callType.GetArgumentsValues(proxyEnv);
		JMethodId methodId = IntCallTests.fixture.Create<JMethodId>();
		...
		JInt result = IntCallTests.fixture.Create<Int32>();
		...
		proxyEnv.CallIntMethod(localRef, methodId, Arg.Is(TestUtilities.GetArgsPtr(args))).Returns(result);
		proxyEnv.CallNonVirtualIntMethod(localRef, Arg.Any<JClassLocalRef>(), methodId,
		                                 Arg.Is(TestUtilities.GetArgsPtr(args))).Returns(result);
		proxyEnv.CallStaticIntMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Is(TestUtilities.GetArgsPtr(args)))
		        .Returns(result);

		Assert.Equal(
			result,
			!isStatic ?
				JFunctionDefinition<JInt>.Invoke(def, jLocal!, jClass, isNonVirtual, args) :
				JFunctionDefinition<JInt>.StaticInvoke(def, jClass!, args));
