...
public sealed class DoubleCallTests
{
		...
		JClassObject? jClass = default)
	{
		...
		proxyEnv.Received(isInstance ? 1 : 0).CallDoubleMethod(localRef, methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
		...
		proxyEnv.Received(isStatic ? 1 : 0)
		        .CallStaticDoubleMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
