...
public sealed class ShortCallTests
{
		...
		JClassObject? jClass = default)
	{
		...
		proxyEnv.Received(isInstance ? 1 : 0).CallShortMethod(localRef, methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
		...
		proxyEnv.Received(isStatic ? 1 : 0)
		        .CallStaticShortMethod(Arg.Any<JClassLocalRef>(), methodId, Arg.Any<ReadOnlyValPtr<JValue>>());
