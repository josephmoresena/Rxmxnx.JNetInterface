...
public partial class JNativeCallAdapterTests
{
	...
	internal void TypedClassParameterCallTest(Boolean useVm, CallResult result = CallResult.Void)
	{
		...
		try
		{
			...
			adapter = adapterBuilder.WithParameter<JClassObject>(classRef.Value, out testClass).Build();
