...
public partial class JNativeCallAdapterTests
{
	...
	internal void TypedStaticParameterlessCallTest(Boolean useVm, CallResult result = CallResult.Void)
	{
		...
		try
		{
			...
			adapter = useVm ?
				JNativeCallAdapter
					.Create<JClassObject>(JVirtualMachine.GetVirtualMachine(proxyEnv.VirtualMachine.Reference),
					                      proxyEnv.Reference, classRef.Value, out testClass).Build() :
				JNativeCallAdapter.Create<JClassObject>(proxyEnv.Reference, classRef.Value, out testClass).Build();
