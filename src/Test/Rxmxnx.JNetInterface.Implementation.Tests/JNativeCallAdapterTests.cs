...
public sealed class JNativeCallAdapterTests
{
	...
	private static readonly WeakReference<ProxyFactory?> agnosticTypedInstanceParameterlessFactory = new(default);
	private static readonly WeakReference<ProxyFactory?> staticParameterlessFactory = new(default);
	private static readonly WeakReference<ProxyFactory?> agnosticStaticParameterlessFactory = new(default);
	...
	private static ProxyFactory AgnosticStaticParameterlessFactory
	{
		get
		{
			if (JNativeCallAdapterTests.agnosticStaticParameterlessFactory.TryGetTarget(out ProxyFactory? result))
				return result;
			result = new(50);
			JNativeCallAdapterTests.agnosticStaticParameterlessFactory.SetTarget(result);
			return result;
		}
	}
	private static ProxyFactory AgnosticTypedInstanceParameterlessFactory
	{
		get
		{
			if (JNativeCallAdapterTests.agnosticTypedInstanceParameterlessFactory
			                           .TryGetTarget(out ProxyFactory? result))
				return result;
			result = new(100);
			JNativeCallAdapterTests.agnosticTypedInstanceParameterlessFactory.SetTarget(result);
			return result;
		}
	}
	...
	{
		NativeInterfaceProxy proxyEnv =
			NativeInterfaceProxy.CreateProxy(JNativeCallAdapterTests.AgnosticTypedInstanceParameterlessFactory);
		JNativeCallAdapter adapter = default;
		JObjectLocalRef localRef = JNativeCallAdapterTests.fixture.Create<JObjectLocalRef>();
		JClassLocalRef classRef = JNativeCallAdapterTests.fixture.Create<JClassLocalRef>();
		JStringLocalRef strRef = JNativeCallAdapterTests.fixture.Create<JStringLocalRef>();
	...
	{
		NativeInterfaceProxy proxyEnv =
			NativeInterfaceProxy.CreateProxy(JNativeCallAdapterTests.AgnosticStaticParameterlessFactory);
		JNativeCallAdapter adapter = default;
		JClassLocalRef classRef = JNativeCallAdapterTests.fixture.Create<JClassLocalRef>();
		JStringLocalRef strRef = JNativeCallAdapterTests.fixture.Create<JStringLocalRef>();
		JStringLocalRef clsStrRef = JNativeCallAdapterTests.fixture.Create<JStringLocalRef>();
		...
		try
		{
			...
			IVirtualMachine vm = JVirtualMachine.GetVirtualMachine(proxyEnv.VirtualMachine.Reference);
			jClass = Assert.IsType<JClassObject>(jLocal);
			Assert.Equal(vm.GetEnvironment(), adapter.Environment);
			proxyEnv.Received(!useVm ? 1 : 0).GetVirtualMachine(Arg.Any<ValPtr<JVirtualMachineRef>>());
			proxyEnv.Received(1).GetObjectClass(classRef.Value);
			proxyEnv.Received(1).CallObjectMethod(classRef.Value, proxyEnv.VirtualMachine.ClassGetNameMethodId,
