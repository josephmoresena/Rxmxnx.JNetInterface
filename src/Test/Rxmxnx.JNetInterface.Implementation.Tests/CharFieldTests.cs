...
public sealed class CharFieldTests
{
	...
	internal void InstanceTest()
	{
		...
		try
		{
			...
			using JLocalObject jLocal = TestUtilities.CreateString(proxyEnv, CharFieldTests.fixture.Create<String>());
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(CharFieldTests.fixture.Create<JClassLocalRef>());
			CharFieldTests.Test(proxyEnv, jLocal);
		}
		finally
		{
	...
	internal void ExtensionTest()
	{
		...
		try
		{
			...
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(CharFieldTests.fixture.Create<JClassLocalRef>());
			CharFieldTests.Test(proxyEnv, jLocal, JClassObject.GetClass<JThrowableObject>(env));
		}
		finally
		{
			JVirtualMachine.RemoveEnvironment(proxyEnv.VirtualMachine.Reference, proxyEnv.Reference);
	...
	internal void StaticTest()
	{
		...
		try
		{
			...
			proxyEnv.FindClass((ReadOnlyValPtr<Byte>)ctx.Pointer)
			        .Returns(CharFieldTests.fixture.Create<JClassLocalRef>());
			CharFieldTests.Test(proxyEnv, default, JClassObject.GetClass<JStackTraceElementObject>(env));
		}
		finally
		{
			JVirtualMachine.RemoveEnvironment(proxyEnv.VirtualMachine.Reference, proxyEnv.Reference);
	...
	{
		JFieldDefinition<JChar> def = new((CString)CharFieldTests.fixture.Create<String>());
		JFieldId fieldId = CharFieldTests.fixture.Create<JFieldId>();
		...
		JChar result = CharFieldTests.fixture.Create<Char>();
		JChar setValue = CharFieldTests.fixture.Create<Char>();
		...
		proxyEnv.GetCharField(localRef, fieldId).Returns(result);
		proxyEnv.GetStaticCharField(Arg.Any<JClassLocalRef>(), fieldId).Returns(result);
		...
		proxyEnv.Received(!isStatic ? 1 : 0).GetCharField(localRef, fieldId);
		proxyEnv.Received(isStatic ? 1 : 0).GetStaticCharField(Arg.Any<JClassLocalRef>(), fieldId);
		...
		proxyEnv.Received(!isStatic ? 1 : 0).SetCharField(localRef, fieldId, setValue);
		proxyEnv.Received(isStatic ? 1 : 0).SetStaticCharField(Arg.Any<JClassLocalRef>(), fieldId, setValue);
