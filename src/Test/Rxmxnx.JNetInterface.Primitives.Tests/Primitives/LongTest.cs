...
public sealed class LongTest : PrimitiveTestBase
{
	...
	private void ConstructorsTest()
	{
		...
		Assert.Equal(-1, new JLong(Double.MaxValue).Value);
		Assert.Equal(0, new JLong(Single.MinValue).Value);
		Assert.Equal(-1, new JLong(Single.MaxValue).Value);
		...
		Assert.Equal(-1, new JLong((Single)Double.MaxValue).Value);
		Assert.Equal(Int32.MinValue, new JLong((Single)Int32.MinValue).Value);
		Assert.Equal(Int32.MaxValue, new JLong((Single)Int32.MaxValue).Value);
		...
		Assert.Equal(-1, new JLong((Double)Single.MaxValue).Value);
